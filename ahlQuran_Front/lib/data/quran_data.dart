// Quran Surahs Data
// This file contains information about all 114 surahs of the Quran
...
Surah? getSurahByNumber(int number) {
  try {
    return quranSurahs.firstWhere((surah) => surah.number == number);
  } catch (e) {
    return null;
  }
}
...
Surah? getSurahByName(String name) {
  try {
    return quranSurahs.firstWhere(
      (surah) => surah.name == name || surah.englishName == name,
    );
  } catch (e) {
    return null;
  }
...
List<Surah> getMeccanSurahs() {
  return quranSurahs
      .where((surah) => surah.revelationType == "Meccan")
      .toList();
}

List<Surah> getMedinanSurahs() {
  return quranSurahs
...
int getTotalAyahs() {
  return quranSurahs.fold(0, (sum, surah) => sum + surah.numberOfAyahs);
}
