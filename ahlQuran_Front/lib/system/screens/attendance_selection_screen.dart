    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      try {
        if (Get.isRegistered<drawer.DrawerController>()) {
          Get.find<drawer.DrawerController>().changeSelectedIndex(9);
        }
      } catch (e) {
  ...
  Widget _buildHeaderBar(ThemeData theme) {
    return Container(
      ...
      child: const Row(
        ...
        children: [
          Text(
            'متابعة المواظبة / الصفحة الرئيسية',
            style: TextStyle(
              color: Colors.white,
              fontSize: 18,
  ...
  Widget _buildSubmitButton() {
    return ElevatedButton(
      onPressed: () {
        ...
        // Navigate to students attendance screen
        Get.toNamed(
          '/attendance/students?lectureId=${selectedLecture!.lectureId}&lectureName=${Uri.encodeComponent(selectedLecture!.lectureNameAr ?? '')}&date=${Uri.encodeComponent(dateController.text)}',
        );
      },
